...
<h1 class="text-center">Gestão de Entrada e Saída de Equipamento - Impressora</h1>
...
function exibirTabela($conexao)
{
    ...
    if (mysqli_num_rows($resultado) > 0) {
        echo '<h2>Entrada Impressora</h2>';
        echo '<table id="impressora-table" class="table table-striped table-bordered">';
...
// Função para exibir os dados da tabela "impressora_saida"
function exibirTabelaImpressoraSaida($conexao)
{
    $sql = "SELECT * FROM impressora_saida";
    $resultado = mysqli_query($conexao, $sql);

    if (mysqli_num_rows($resultado) > 0) {
        echo '<h2>Saida Impressora</h2>';
        echo '<table id="impressora_saida-table" class="table table-striped table-bordered">';
        echo '<thead><tr><th>IMPRESSORA_ID</th><th>Marca</th><th>Modelo</th><th>Série</th><th>Tipo</th><th>Quantidade</th><th>Obs</th><th>Ações</th></tr></thead>';
        echo '<tbody>';

        while ($row = mysqli_fetch_assoc($resultado)) {
            echo '<tr>';
            echo '<td>' . $row['IMPRESSORA_ID'] . '</td>';
            echo '<td>' . $row['MARCA'] . '</td>';
            echo '<td>' . $row['MODELO'] . '</td>';
            echo '<td>' . $row['SERIE'] . '</td>';
            echo '<td>' . $row['TIPO'] . '</td>';
...
if (mysqli_connect_errno()) {
    ...
    if (isset($_POST['submit_insert'])) {
        ...
        $PROCESSADOR = mysqli_real_escape_string($conexao, $_POST['TIPO']);
        ...
        if (empty($MARCA) || empty($MODELO) || empty($SERIE) || empty($TIPO) || empty($QUANTIDADE) || empty($OBS)) {
        ...
        if (count($errors) > 0) {
            ...
            $sql_check_quantity = "SELECT QUANTIDADE FROM impressora WHERE QUANTIDADE >= ?";
            ...
            if (mysqli_num_rows($resultado_check_quantity) > 0) {
                // Insere o registro na tabela "impressora_saida"
                $sql_insert_saida = "INSERT INTO impressora_saida (MARCA, MODELO, SERIE, TIPO, QUANTIDADE, OBS) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
                $stmt_insert_saida = mysqli_prepare($conexao, $sql_insert_saida);
                mysqli_stmt_bind_param($stmt_insert_saida, "ssssssis", $marca, $MODELO, $SERIE, $TIPO, $QUANTIDADE, $OBS);

                if (mysqli_stmt_execute($stmt_insert_saida)) {
                    // Atualiza a quantidade na tabela "impressora"
                    $sql_update_impressora = "UPDATE impressora SET QUANTIDADE = QUANTIDADE - ? WHERE QUANTIDADE >= ?";
                    $stmt_update_impressora = mysqli_prepare($conexao, $sql_update_impressora);
                    mysqli_stmt_bind_param($stmt_update_impressora, "ii", $QUANTIDADE, $QUANTIDADE);
                    mysqli_stmt_execute($stmt_update_impressora);
    ...
    exibirTabelaImpressoraSaida($conexao);
...
<script>
    $(document).ready(function() {
        ...
        $('#impressora-table').DataTable();
        // Use o ID correto para a tabela "cpu_saida"
        $('#impressora_saida-table').DataTable(); 
        ...
        $('.btn-editar').click(function() {
            var row = $(this).closest('tr');
            var idimpressora = row.find('td:first').text();

            // Abre uma nova janela com os campos dimensionados para a edição
            window.open('impressora_editar.php?id=' + idimpressora, '_blank', 'width=800,height=300');
        });
