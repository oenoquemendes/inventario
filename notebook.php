...
<h1 class="text-center">Gestão de Entrada e Saída de Equipamento - Notebook</h1>
...
function exibirTabela($conexao)
{
    ...
    if (mysqli_num_rows($resultado) > 0) {
        echo '<h2>Entrada Notebook</h2>';
        echo '<table id="notebook-table" class="table table-striped table-bordered">';
...
function exibirTabelaNotebookSaida($conexao)
{
    $sql = "SELECT * FROM notebook_saida";
    $resultado = mysqli_query($conexao, $sql);

    if (mysqli_num_rows($resultado) > 0) {
        echo '<h2>Saida Notebook</h2>';
        echo '<table id="notebook_saida-table" class="table table-striped table-bordered">';
        echo '<thead><tr><th>NOTEBOOK_ID</th><th>Marca</th><th>Modelo</th><th>Série</th><th>Processador</th><th>Memória Ram</th><th>Armazenamento</th><th>Quantidade</th><th>Obs</th><th>Ações</th></tr></thead>';
        echo '<tbody>';

        while ($row = mysqli_fetch_assoc($resultado)) {
            echo '<tr>';
            echo '<td>' . $row['NOTEBOOK_ID'] . '</td>';
            echo '<td>' . $row['MARCA'] . '</td>';
            echo '<td>' . $row['MODELO'] . '</td>';
            echo '<td>' . $row['SERIE'] . '</td>';
            echo '<td>' . $row['PROCESSADOR'] . '</td>';
...
if (mysqli_connect_errno()) {
    ...
    if (isset($_POST['submit_insert'])) {
        ...
        if (count($errors) > 0) {
            ...
            $sql_check_quantity = "SELECT QUANTIDADE FROM notebook WHERE QUANTIDADE >= ?";
            ...
            if (mysqli_num_rows($resultado_check_quantity) > 0) {
                // Insere o registro na tabela "notebook_saida"
                $sql_insert_saida = "INSERT INTO notebook_saida (MARCA, MODELO, SERIE, PROCESSADOR, MEMORIA_RAM, ARMAZENAMENTO, QUANTIDADE, OBS) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
                ...
                if (mysqli_stmt_execute($stmt_insert_saida)) {
                    // Atualiza a quantidade na tabela "notebook"
                    $sql_update_notebook = "UPDATE notebook SET QUANTIDADE = QUANTIDADE - ? WHERE QUANTIDADE >= ?";
                    $stmt_update_notebook = mysqli_prepare($conexao, $sql_update_notebook);
                    mysqli_stmt_bind_param($stmt_update_notebook, "ii", $QUANTIDADE, $QUANTIDADE);
                    mysqli_stmt_execute($stmt_update_notebook);
    ...
    exibirTabelaNotebookSaida($conexao);
...
<script>
    $(document).ready(function() {
        ...
        $('#notebook-table').DataTable();
        // Use o ID correto para a tabela "cpu_saida"
        $('#notebook_saida-table').DataTable(); 
        ...
        $('.btn-editar').click(function() {
            var row = $(this).closest('tr');
            var idnotebook = row.find('td:first').text();

            // Abre uma nova janela com os campos dimensionados para a edição
            window.open('notebook_editar.php?id=' + idnotebook, '_blank', 'width=800,height=300');
        });
